...
|------------|-------|---------|
| 状態を持つ | useState（自動で再描画） | 普通の変数 |
| 再描画 | 自動（バッチ最適化あり） | this.update()（手動） |
| 副作用 | useEffect（依存配列で制御） | そのまま書く |
| クリーンアップ | return 関数 | AbortSignal |
| 状態の共有 | Context（自動で再描画） | EventTarget + this.on() |
...
トレードオフはある。React の自動化を手放す代わりに、仕組みが見える。

</v-click>

<!--
【話すこと】
React 側に「自動」って書いてますけど、これって React がわざわざそうしてる理由があるんですよね。
バッチ処理とか、不要な再レンダリングを防ぐとか。
Remix 3 はその部分を開発者に委ねることで、コードの見た目をシンプルにした。
どっちが良いかは好みと用途次第です。
-->
